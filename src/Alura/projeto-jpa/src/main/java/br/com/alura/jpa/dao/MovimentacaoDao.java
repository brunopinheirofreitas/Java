...
public class MovimentacaoDao {
	...
	public List<MediaComData> getMediaDiariaDasMovimentacoes() {	
		...
		//String jqpl = "select new br.com.alura.jpa.modelo.MediaComData(avg(m.valor), day(m.data), month(m.data)) from Movimentacao m group by day(m.data), month(m.data)";
	...
	public BigDecimal getSomaDasMovimentacoes() {
		...
		TypedQuery<BigDecimal> query = em.createNamedQuery("mediaGeralMovimentacoes", BigDecimal.class);
	...
	public List<Movimentacao> getMovimentacoesFiltradasPorData(Integer dia, Integer mes, Integer ano) {
		...
		query.where( (Predicate[]) predicates.toArray(new Predicate[0])); //Performance. Array's size has the size of the return query.
